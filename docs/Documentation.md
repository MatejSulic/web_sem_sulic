...
- Doplnit validaci vstupů (max. délky, povolené znaky) a centralizovanou sanitizaci pro všechny formuláře.
- Přidat CSRF tokeny do formulářů (změna rolí, upload, recenze).
- Ošetřit kontrolu MIME typu u uploadu a limit velikosti souboru.
- Přidat logování chyb místo `die()` a přesměrování na chybové stránky.
- Dolaďit práva tak, aby autor nemohl editovat publikované články (momentálně hlídá pouze stav `pending`).
- Přidat stránkování/filtry do přehledů a lepší práci s průměrným hodnocením.
